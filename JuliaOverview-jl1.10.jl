...
<a href="https://www.ipam.ucla.edu/programs/long-programs/bridging-the-gap-transitioning-from-deterministic-to-stochastic-interaction-modeling-in-electrochemistry/">
IPAM Long Program Bridging the Gap: Transitioning from Deterministic to Stochastic Interaction Modeling in Electrochemistry
</a><br>
...
md"""
The notebook code can be found on [https://github.com/j-fu/JuliaIPAM2025](https://github.com/j-fu/JuliaIPAM2025)
"""
...
```
  ...
  - Current version: 1.12 (has longer startup times)
"""
...
htl"""
<h2> <a href="https://modernjuliaworkflows.org/">Modern Julia Workflows</a></h2>
<iframe src="https://modernjuliaworkflows.org/" style="width: 600px; height: 300px;"></iframe>"""

# ╔═╡ dbf21ad2-1a9b-418e-93e5-08da11649b54
htl""" 
<h2> <a href="https://techytok.com/from-zero-to-julia/">From Zero to Julia!</a> </h2>
<iframe src="https://techytok.com/from-zero-to-julia/" style="width: 600px; height: 300px;"></iframe>"""
...
- Some interesting packages:
   - Flux.jl, Lux.jl, MLJ.jl ``\dots``: Neural networks
   - GLMakie.jl: Plotting with GPU support, CairoMakie.jl 
   - PyPlot.jl: Interface to python/matplotlib
   - ForwardDiff.jl, Mooncake.jl, DifferentiationInterface.jl: Automatic differentiation
   - ModelingToolkit.jl: Modeling framework with integrated symbolics for physics-informed machine learning and automated transformations of differential equations
   - Turing.jl: Probabilistic programming.
   - Distributions.jl: Probability distributions and associated functions.
   - KernelAbstractions.jl, CUDA.jl, CUArrays.jl: Julia on GPU 
   - Gridap.jl: Finite element package
   - Bifurcationkit.jl: Bifurcation analysis
   - DFTK.jl, Molly.jl: DFT and MD in Julia
"""
...
# ╔═╡ 960f273a-f9ef-41f4-81b0-11c687ee51d6
md"""
### Multiple Dispatch
"""
...
md"""
Julia generates specialized code for function methods depending on the type of function parameters  __if the types of the parametes can be inferred__
...
# ╔═╡ a4b632e0-a863-4188-bf41-9f267bbcd67b
md"""
...
_x=1;@code_native square(1.0);

# ╔═╡ 8655380e-c20b-48da-a898-7dca6f1e710f
_x;MethodAnalysis.methodinstances(square)
...
# ╔═╡ 53baef6e-9cab-4547-a0c2-792f21020f54
x=10.5
...
- Automatic solver detection (when loading DifferentialEquations.jl which loads all nearly all possible methods)
...
# ╔═╡ 0a53ef3e-9168-4ba4-9a41-59420175b913
u0 = [1.0;0.0;.0];	

# ╔═╡ 8b5d3ab4-db36-4e8c-96be-2c31d593a4dd
tspan = (0.0,50.0)

# ╔═╡ 66763ecf-4e20-46ce-8a6b-c5e8005f5895
problem=ODEProblem(lorenz!,u0,tspan)

# ╔═╡ 62e81c82-dc40-4a27-af7f-7d01e3eba002
sol=solve(problem,RK4())

# ╔═╡ 83feccc8-d9c8-43ae-afa1-7594e329d92b
plot(sol,idxs=(3,1,2),size=(300,300))
...
md"""
## Some Julia Caveats
- Inference JIT precompilation time 
    - the price for generic programming and  multiple dispatch
    - this has improved significantly between  v1.0 and v1.10 with a time increase towards current v1.12 due to significant internal restructuring
...
manifest_format = "2.0"
project_hash = "7c987163802bae208f3f3db7563acf9ae1f15e0b"
...
git-tree-sha1 = "4a8d84be205946f464e8300c6e44ba5c97c2db5d"
